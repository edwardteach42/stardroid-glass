...
public class DynamicStarMapActivity //
	...
	public void onCreate(Bundle icicle) {
		...
		boolean eulaConfirmed = sharedPreferences.getBoolean(READ_TOS_PREF,
				false);
		if (!eulaConfirmed) {
			new AlertDialog.Builder(this)
					.setMessage(
							"Do you agree to the Terms and Conditions located at:\nhttp://m.google.com/tospage")
					.setCancelable(false)
							...
							new DialogInterface.OnClickListener() {
								public void onClick(DialogInterface dialog,
									...
									SharedPreferences sharedPreferences = PreferenceManager
											.getDefaultSharedPreferences(DynamicStarMapActivity.this);
									Editor editor = sharedPreferences.edit();
									editor.putBoolean(
											DynamicStarMapActivity.READ_TOS_PREF,
											true);
									editor.commit();
								}
							})
		...
		layerManager = StardroidApplication.getLayerManager(getAssets(),
				sharedPreferences, getResources(), this);
	 ...
	 * (keyCode) { case (KeyEvent.KEYCODE_DPAD_LEFT): Log.d(TAG, "Key left");
	 * controller.rotate(-10.0f); break; case (KeyEvent.KEYCODE_DPAD_RIGHT):
	 * Log.d(TAG, "Key right"); controller.rotate(10.0f); break; case
	 * (KeyEvent.KEYCODE_BACK): // If we're in search mode when the user presses
	 * 'back' the natural // thing is to back out of search. Log.d(TAG,
	 * "In search mode " + searchMode); if (searchMode) { cancelSearch(); break;
	 * } default: Log.d(TAG, "Key: " + event); return super.onKeyDown(keyCode,
	 * event); } return true; }
	 */
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		/*
		 * case R.id.menu_item_search: Log.d(TAG, "Search");
		 * Analytics.getInstance
		 * (this).trackEvent(Analytics.USER_ACTION_CATEGORY,
		...
		case R.id.menu_item_settings:
			...
			Analytics.getInstance(this).trackEvent(
					Analytics.USER_ACTION_CATEGORY, Analytics.MENU_ITEM,
					Analytics.SETTINGS_OPENED_LABEL, 1);
		...
		case R.id.menu_item_dim:
			...
			sharedPreferences
					.edit()
					.putString(ActivityLightLevelManager.LIGHT_MODE_KEY,
							nightMode ? "NIGHT" : "DAY").commit();
			Analytics.getInstance(this).trackEvent(
					Analytics.USER_ACTION_CATEGORY, Analytics.MENU_ITEM,
					Analytics.TOGGLED_NIGHT_MODE_LABEL, nightMode ? 1 : 0);
			break;
		/*
		 * case R.id.menu_item_time: Log.d(TAG,
		 * "Starting Time Dialog from menu");
		 * Analytics.getInstance(this).trackEvent
		 * (Analytics.USER_ACTION_CATEGORY, Analytics.MENU_ITEM,
		 * Analytics.TIME_TRAVEL_OPENED_LABEL, 1); if (!timePlayerUI.isShown())
		 * { Log.d(TAG, "Resetting time in time travel dialog.");
		 * controller.goTimeTravel(new Date()); } else { Log.d(TAG,
		 * "Resuming current time travel dialog."); }
		 * showDialog(DialogFactory.DIALOG_ID_TIME_TRAVEL); break; case
		 * R.id.menu_item_gallery: Log.d(TAG, "Loading gallery");
		 * Analytics.getInstance
		 * (this).trackEvent(Analytics.USER_ACTION_CATEGORY,
		 * Analytics.MENU_ITEM, Analytics.GALLERY_OPENED_LABEL, 1);
		 * startActivity(new Intent(this, ImageGalleryActivity.class)); break;
		 */
		case R.id.menu_item_tos:
			Log.d(TAG, "Loading ToS");
			Analytics.getInstance(this).trackEvent(
					Analytics.USER_ACTION_CATEGORY, Analytics.MENU_ITEM,
					Analytics.TOS_OPENED_LABEL, 1);
	...
	public void onStart() {
		super.onStart();
		Analytics.getInstance(this).trackPageView(
				Analytics.DYNAMIC_STARMAP_ACTIVITY);
	...
	public void onStop() {
		...
		Analytics.getInstance(this).trackEvent(Analytics.GENERAL_CATEGORY,
				Analytics.SESSION_LENGTH_BUCKET, bucket.toString(),
	...
	/*
	 * @Override public boolean onTrackballEvent(MotionEvent event) { Log.d(TAG,
	 * "Trackball motion " + event); controller.rotate(event.getX() *
	 * ROTATION_SPEED); return true; }
	 */
	private void doSearchWithIntent(Intent searchIntent) {
		...
		Analytics.getInstance(this).trackEvent(Analytics.USER_ACTION_CATEGORY,
				Analytics.SEARCH, "search:" + queryString,
				results.size() > 0 ? 1 : 0);
		if (results.size() == 0) {
	...
	/*
	 * private void wireUpScreenControls() { cancelSearchButton = (ImageButton)
	 * findViewById(R.id.cancel_search_button); // TODO(johntaylor): move to set
	 * this in the XML once we don't support 1.5
	 * cancelSearchButton.setOnClickListener(new OnClickListener() {
	 * 
	 * @Override public void onClick(View v) { cancelSearch(); } });
	 * 
	 * final ZoomControls zooms = (ZoomControls)
	 * findViewById(R.id.zoom_control); final WidgetFader zoomControlFader = new
	 * WidgetFader(new Fadeable() {
	 * 
	 * @Override public void hide() { zooms.hide(); }
	 * 
	 * @Override public void show() { zooms.show(); } });
	 * zooms.setOnZoomInClickListener(new OnClickListener() {
	 * 
	 * @Override public void onClick(View v) { controller.zoomIn();
	 * zoomControlFader.keepActive(); } }); zooms.setOnZoomOutClickListener(new
	 * OnClickListener() {
	 * 
	 * @Override public void onClick(View v) { controller.zoomOut();
	 * zoomControlFader.keepActive(); } });
	 * zooms.setZoomSpeed(DELAY_BETWEEN_ZOOM_REPEATS_MILLIS); zooms.hide();
	 * final ButtonLayerView providerButtons = (ButtonLayerView)
	 * findViewById(R.id.layer_buttons_control); final WidgetFader
	 * layerControlFader = new WidgetFader(providerButtons, 2500);
	 * providerButtons.hide(); final int numChildren =
	 * providerButtons.getChildCount(); for (int i = 0; i < numChildren; ++i) {
	 * final ImageButton button = (ImageButton) providerButtons.getChildAt(i);
	 * button.setOnClickListener(new OnClickListener() {
	 * 
	 * @Override public void onClick(View v) { layerControlFader.keepActive(); }
	 * }); } final ButtonLayerView manualButtonLayer = (ButtonLayerView)
	 * findViewById(R.id.layer_manual_auto_toggle); final WidgetFader
	 * manualControlFader = new WidgetFader(manualButtonLayer);
	 * manualButtonLayer.hide(); final ImageButton manualAuto = (ImageButton)
	 * findViewById(R.id.manual_auto_toggle); manualAuto.setOnClickListener(new
	 * OnClickListener() {
	 * 
	 * @Override public void onClick(View v) { manualControlFader.keepActive();
	 * } });
	 * 
	 * MapMover mapMover = new MapMover(model, controller, this,
	 * sharedPreferences); gestureDetector = new GestureDetector(new
	 * GestureInterpreter( new WidgetFader[] {manualControlFader,
	 * layerControlFader, zoomControlFader}, mapMover)); dragZoomRotateDetector
	 * = new DragRotateZoomGestureDetector(mapMover); }
	 */
	...
	private void wireUpTimePlayer() {
		...
		Runnable displayUpdater = new Runnable() {
			private final TextView timeTravelTimeReadout = (TextView) findViewById(R.id.time_travel_time_readout);
			private final TextView timeTravelStatusLabel = (TextView) findViewById(R.id.time_travel_status_label);
			private final TextView timeTravelSpeedLabel = (TextView) findViewById(R.id.time_travel_speed_label);
